...
/***/ function(module, exports) {
	...
	var build = function initTheCalAndRender(first, second) {
	  ...
	  if (mode === 2) {
	    $('.mvDdigit').eq(dCount).addClass('selectedDayDigit');
	  }
	  ...
	  */
	  ...
	  // toggle month/year on title click
	  $('#macroContent').listen('click', function () {
	    if (mode === 0) {
	      mode = 2;
	    } else {
	      mode = 0;
	  ...
	  $('.mvDayCell').listen('click', function listenToDayInM() {
	    ...
	    if (dCount < 9 && mCount < 9) {
	      $('#js-date').val('0' + calLib[count].days[mCount][dCount] + '/0' + (mCount + 1) + '/' + calLib[count].year);
	      $('#dateToDB').val(calLib[count].year + '0' + (mCount + 1) + '0' + calLib[count].days[mCount][dCount]);
	    } else if (dCount < 9) {
	      $('#js-date').val('0' + calLib[count].days[mCount][dCount] + '/' + (mCount + 1) + '/' + calLib[count].year);
	      $('#dateToDB').val('' + calLib[count].year + (mCount + 1) + '0' + calLib[count].days[mCount][dCount]);
	    } else if (mCount < 9) {
	      $('#js-date').val(calLib[count].days[mCount][dCount] + '/0' + (mCount + 1) + '/' + calLib[count].year);
	      $('#dateToDB').val(calLib[count].year + '0' + (mCount + 1) + calLib[count].days[mCount][dCount]);
	    } else {
	      $('#js-date').val(calLib[count].days[mCount][dCount] + '/' + (mCount + 1) + '/' + calLib[count].year);
	      $('#dateToDB').val('' + calLib[count].year + (mCount + 1) + calLib[count].days[mCount][dCount]);
	    }
	    $('p').removeClass('selectedDayDigit');
	    this.childNodes[1].classList.add('selectedDayDigit');

	    $('#calendarProper').addClass('vanish');
	...
	var init = function setUpBaseView() {
	  calInit(80);
	  // build('#macroContent', '#microContent');
	};
	...
	$('#mainCalendar').listen('click', function () {
	  build('#macroContent', '#microContent');
	  $('#calendarProper').removeClass('vanish');
	  $('#lightbox').removeClass('vanish');
	});

	$('#lightbox').listen('click', function () {
	  $('#calendarProper').addClass('vanish');
	  $('#lightbox').addClass('vanish');
	});
	// console.log(mode);
...
/***/ function(module, exports) {
	...
	function init() {
	  ...
	  //loads the surch field file
	  function add_field(file) {
	    $('#s_fields').ajax({ url: "../include/include_" + file + ".html" });
	    $('#add_' + file).css('display:none');
	  }

	  //adds a search field corresponding to the button clicked
	  $('#add_date').listen('click', function () {
	    add_field('date');
	  });
	  $('#add_type').listen('click', function () {
	    add_field('type');
	  });
	  $('#add_service').listen('click', function () {
	    add_field('service');
	  });
	  $('#add_saisie').listen('click', function () {
	    add_field('saisie');
	  });
	  $('#add_status').listen('click', function () {
	    add_field('status');
	  });

	  //toggle the + on the add buttons between white and green
	  // to green on in
	  $('.add_section').listen('mouseenter', function () {
	    this.childNodes[1].childNodes[0].setAttribute('src', 'img/add_green.png');
	  });
	  // to white on out
	  $('.add_section').listen('mouseleave', function () {
	    this.childNodes[1].childNodes[0].setAttribute('src', 'img/add.png');
	  });

	  // get the searchresult into a string
	  var vault = $('#test_echo').val();

	  // split the data in an array by arretes
	  var arrTab = vault.split("-|-");
	  var arrTabDet = [];

	  // split each arrete by section
	  for (var i = 0; i < arrTab.length; i++) {
	    arrTabDet[i] = arrTab[i].split("-:-");
	  }

	  function populate() {
	    for (var i = 0; i < arrTab.length - 1; i++) {
	      $("#resultField").append('<div class="res_bar res2"><input type="checkbox" class="check"><p class="res_cat res">' + arrTabDet[i][0] + '</p><p class="res_cat res">' + arrTabDet[i][1] + '</p><p class="res_cat res">' + arrTabDet[i][2] + '</p><p class="res_cat res">' + arrTabDet[i][3] + '</p><p class="res_cat res">' + arrTabDet[i][4] + '</p></div><hr>');
	    }
	  }

	  populate();
	  // console.log(arrTabDet);
	}
